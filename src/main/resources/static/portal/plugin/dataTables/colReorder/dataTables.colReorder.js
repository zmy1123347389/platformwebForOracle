	 ...
	 *    } );
        ...
        ColReorder.defaults = {
            ...
            fnReorderCallback: function(){
                var _this=this;
                var $tableKey= $("#"+this.s.dt.sTableId).attr("data-tableKey");
                if($tableKey){
                    var $tableWrap = $("#" + this.s.dt.sTableId + "_wrapper");
                    var columnArr = this.s.dt.aoColumns;
                    $("li", $tableWrap).each(function (indexs, items) {
                        var $name = $(items).attr("name");
                        var $value=$(items).attr("value");
                        $.each(columnArr, function (index, item) {
                            if ($value == item.idx && $name != item.data) {
                                var $visible = item.bVisible;
                                $(items).html('<span ></span>' + item.sTitle);
                                $(items).attr("name",item.data);
                                $(items).attr("value",item.idx);
                                $(items).attr("title",item.sTitle);
                                var column = $("#"+_this.s.dt.sTableId).DataTable().column(item.idx);
                                if($visible){
                                    column.visible(true);
                                    $(items).removeClass("hideColCheck");
                                }else{
                                    column.visible(false);
                                    $(items).addClass("hideColCheck");
                                }
                            }
                        });
                    });
